      ...
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        if (state.history.isNotEmpty) {
          ...
          children.add(AudioSource.uri(
            Uri.parse(_api.getStreamUrl(prev.id)),
            tag: MediaItem(
              id: prev.id,
              album: prev.album,
              title: prev.title,
              artist: prev.artist,
              artUri: Uri.parse(_api.getCoverUrl(prev.albumId ?? prev.id, size: 500)),
            ),
          ));
          initialIndex = 1;
        }
        ...
        children.add(AudioSource.uri(
          ...
          tag: MediaItem(
            ...
            title: song.title,
            artist: song.artist,
            artUri: Uri.parse(_api.getCoverUrl(song.albumId ?? song.id, size: 500)),
          ),
        ));
        
        // 3. Next Track
        if (state.queue.isNotEmpty) {
          final nxt = state.queue.first;
          children.add(AudioSource.uri(
            Uri.parse(_api.getStreamUrl(nxt.id)),
            tag: MediaItem(
              id: nxt.id,
              album: nxt.album,
              title: nxt.title,
              artist: nxt.artist,
              artUri: Uri.parse(_api.getCoverUrl(nxt.albumId ?? nxt.id, size: 500)),
            ),
          ));
        }

        audioSource = ConcatenatingAudioSource(children: children);
      } else {
        // Standard single AudioSource for Desktop/Web
